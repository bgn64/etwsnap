...
public interface IPipeClient : IDisposable
{
    ...
    Task<Response> SendCommandWithProgressAsync(Request request, Action<Response> onProgress, CancellationToken cancellationToken = default);
...
public class NamedPipeClient : IPipeClient
{
    ...
    public async Task<Response> SendCommandWithProgressAsync(Request request, Action<Response> onProgress, CancellationToken cancellationToken = default)
    {
        try
        {
            using var client = new System.IO.Pipes.NamedPipeClientStream(".", _pipePath, System.IO.Pipes.PipeDirection.InOut);
            ...
            // Read multiple responses (progress updates + final response)
            using var reader = new StreamReader(client, leaveOpen: true);
            ...
            while (true)
            {
                ...
                // If this is a progress update, call the callback
            ...
            { 
                Status = ResponseStatus.Error, 
                Message = "No final response from service" 
            };
        }
        catch (TimeoutException)
        {
